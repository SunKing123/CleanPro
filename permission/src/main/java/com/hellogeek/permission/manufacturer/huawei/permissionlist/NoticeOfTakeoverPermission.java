...
public class NoticeOfTakeoverPermission extends ManfacturerBase {
    ...
    public void openNoticeOfTakeover(Context mContext, AccessibilityNodeInfo info, AccessibilityService service) {
        if (AccessibilitUtil.isOpenPermission(mContext, Permission.NOTICEOFTAKEOVER)&&!mIsOpen) {
            ...
            return;
        }else if (NodeInfoUtil.pageContains(info, "读取应用通知")||NodeInfoUtil.pageContains(info, "通知使用权") || NodeInfoUtil.pageContains(info, "通知读取") && !getList().contains(SIGN1)) {
        ...
        } else if (NodeInfoUtil.pageContains(info, "允许") || NodeInfoUtil.pageContains(info, AppUtils.getAppName(mContext)) && getList().contains(SIGN1) && !getList().contains(SIGN2)) {
            boolean isOpen1 = NodeInfoUtil.clickNodeInfoAll(mContext,info, "允许");
            if (isOpen1) {
                mIsOpen = true;
                PermissionProvider.save(mContext, PROVIDER_NOTICEOFTAKEOVER, true);
                EventBus.getDefault().post(new PathEvent(Permission.NOTICEOFTAKEOVER, true,1));
        ...
        } else if (NodeInfoUtil.pageContains(info, "授予此权限") || NodeInfoUtil.pageContains(info, "读取系统或任何已安装的应用") && getList().contains(SIGN1) && !getList().contains(SIGN2)) {
