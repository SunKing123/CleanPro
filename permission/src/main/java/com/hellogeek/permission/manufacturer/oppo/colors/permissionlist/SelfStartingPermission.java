...
public class SelfStartingPermission extends ManfacturerBase {
    ...
    public void openSelfStarting(Context mContext, AccessibilityNodeInfo info, AccessibilityService service) {
        if (AccessibilitUtil.isOpenPermission(mContext, Permission.SELFSTARTING)&&!mIsOpen) {
            ...
            addSIGN(SIGN1);
        } else if ((NodeInfoUtil.pageContains(info, "自启动管理") && NodeInfoUtil.pageContains(info, AppUtils.getAppName(mContext))) &&
                getList().contains(SIGN1)) {
            ...
            if (!NodeInfoUtil.pageContains(info, "自启动管理")){
                return;
            }
    ...
    public void openSelfStartingV2V3V4(Context mContext, AccessibilityNodeInfo info, AccessibilityService service) {
        if (AccessibilitUtil.isOpenPermission(mContext, Permission.SELFSTARTING)&&!mIsOpen) {
            if (!result) {
                result = true;
                EventBus.getDefault().post(new PathEvent(Permission.SELFSTARTING, true,0));
            ...
            return;
        }else if (NodeInfoUtil.pageContains(info, "应用信息") &&
                NodeInfoUtil.pageContains(info, AppUtils.getAppName(mContext)) && !getList().contains(SIGN1)) {
            if (NodeInfoUtil.pageContains(info, "允许自动启动")) {
                boolean isOpen = NodeInfoUtil.clickNodeInfoAll(mContext, info, "允许自动启动");
                addSIGN(SIGN1);
                if (isOpen) {
                    mIsOpen = isOpen;
                    PermissionProvider.save(mContext, PROVIDER_SELFSTARTING, true);
        ...
        } else if (NodeInfoUtil.pageContains(info, AppUtils.getAppName(mContext)) && !getList().contains(SIGN1)) {
            boolean isOpen = NodeInfoUtil.clickNodeInfoAll(mContext, info, AppUtils.getAppName(mContext));
            addSIGN(SIGN1);
            if (isOpen) {
                mIsOpen = isOpen;
