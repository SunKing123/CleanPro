...
public class JKQSVideoView extends QSVideoViewHelp {
    ...
    protected ViewGroup retryContainer;//出错了显示的 重试
    ...
    public JKQSVideoView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);

    }

    public JKQSVideoView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void initView() {
        ...
        retryContainer = (ViewGroup) findViewById(R.id.retry_container);
        ...
        changeViews.add(retryContainer);
    ...
    protected void changeUiWithStateAndMode(int status, int mode) {
        Log.i("123", "changeUiWithStateAndMode:" + status + "_" + mode);
        switch (status) {
            case STATE_NORMAL:
                showChangeViews(coverImageView, startButton);//普通状态显示封面和播放按钮
                break;
            case STATE_PREPARING:
                showChangeViews(coverImageView, loadingContainer);//初始化状态显示loading
                break;
            case STATE_PLAYING:
            case STATE_PAUSE://显示 播放按钮  [底部] [顶部]
                showChangeViews(startButton, mFirstPlay ? coverImageView : null,
            ...
            case STATE_AUTO_COMPLETE:
                showChangeViews(coverImageView, retryContainer);
                retryText.setText("重播");
                break;
            case STATE_ERROR://出错显示errorContainer
                Proxy.deleteCacheVideo();
                showChangeViews(coverImageView, retryContainer);
                retryText.setText("视频加载失败，点击重试");
                break;
        }
