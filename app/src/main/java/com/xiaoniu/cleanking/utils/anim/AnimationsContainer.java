 ...
 * 读取帧动画的工具类
 * 使用方法
 * FrameseAnim animaDra = AnimationsContainer.getInstance(R.array.small_ad_anim, 80).createAnim(mLlAdAnim);
 * animaDra.start();
 * animaDra.stop();
 */
...
public class AnimationsContainer {
    ...
    public static AnimationsContainer getInstance(int resId, int fps) {
        ...
        mInstance.setResId(resId, fps, true);
        return mInstance;
    }

    //获取单例
    public static AnimationsContainer getInstance(int resId, int fps, boolean mIsRepeat) {
        mInstance = new AnimationsContainer();
        mInstance.setResId(resId, fps, mIsRepeat);
        return mInstance;
    }

    public void setResId(int resId, int fps, boolean mIsRepeat) {
        this.resId = resId;
        this.FPS = fps;
     ...
     */
    FrameseAnim frameseAnim;

    public FrameseAnim createAnim(ImageView imageView) {
        frameseAnim = new FrameseAnim(imageView, getData(resId), FPS);
        return frameseAnim;
    }
    ...
    public boolean isRuning() {
        ...
        return frameseAnim.isRunning();
    }

    /**
     * 循环读取帧---循环播放帧
     */
    public class FrameseAnim {
        ...
        public FrameseAnim(ImageView imageView, int[] frames, int fps) {
        ...
        private int getNext() {
            mIndex++;
            if (mIndex >= mFrames.length) {//动画一圈了
                if (mIsRepeat) {
                    mIndex = 0;
                } else {//取最后一帧
                    stop();
