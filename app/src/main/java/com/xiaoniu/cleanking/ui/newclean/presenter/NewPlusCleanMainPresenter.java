...
//                getView().setScanningFileCount(fileCount);
    ...
    public void prepareVideoAd(ViewGroup viewGroup) {
        ...
        //尝试预加载，丝滑般的体验...
        MidasAdSdk.getAdsManger().askIsReady(mView.getActivity(), MidasConstants.MAIN_THREE_AD_ID, new AskReadyCallBack() {
            @Override
            public void onReady(boolean result) {
                isReadSuccess = result;
                LogUtils.e("===========视频广告预加载成功");
            }
        });
    }

    public void fillVideoAd(ViewGroup viewGroup) {
        AdRequestParams params = new AdRequestParams.Builder()
                .setAdId(MidasConstants.MAIN_THREE_AD_ID).setActivity(mView.getActivity())
                .setViewContainer(viewGroup).build();
        MidasRequesCenter.requestAd(params, new AdvCallBack(MidasConstants.MAIN_THREE_AD_ID));
    }
