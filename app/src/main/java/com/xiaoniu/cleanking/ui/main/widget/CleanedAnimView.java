...
public class CleanedAnimView extends RelativeLayout {
    ...
    public CleanedAnimView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }
    ...
    public void initView(Context context) {
        ...
        mContext = context;
        View v = LayoutInflater.from(mContext).inflate(R.layout.layout_cleaned_anim, this, true);
...
//        mTextGb.setText(mCountEntity.getUnit());
    ...
    public void startMiddleAnim(boolean isNeedTranslation) {
        try {
            //位移的距离
            int height = ScreenUtils.getScreenHeight(AppApplication.getInstance()) / 2 - DisplayUtils.dip2px(170);
    ...
    public void setViewTrans() {
        ...
        anim.setInterpolator(new AccelerateDecelerateInterpolator());
        LayoutParams rlp = (LayoutParams) mLayoutCleanFinish.getLayoutParams();
        anim.addUpdateListener(animation -> {
            int currentValue = (int) animation.getAnimatedValue();
